...
| `<module>-<version>-content-graph.json` | Launcher, runtimes, review tools (Release-Index `content-graph` artifact role) |
| `content-graph-evidence.json` | Release Index, Website, Mobile Studio, runtime QA gates (Release-Index `content-graph-evidence` artifact role) |
...
| `.echo/content-graph/*` | Embedded in every runtime archive; mirrors the content-graph sidecar |
...
When available, it also exposes the release-root `content-graph-evidence.json` artifact so public surfaces can show the same graph, module, node, edge, feature, export-plan, and Hytale blocker counts without parsing every module sidecar.
